...
$schemas = [
    ...
    'purchases' => ['id', 'type', 'date', 'supplierId', 'supplierName', 'description', 'items', 'totalAmount', 'amountPaid', 'paymentStatus', 'paymentMethod', 'bankId', 'bankName', 'paymentHistory', 'createdAt', 'updatedAt'],
...
switch ($method) {
    case 'GET':
        if ($id) {
            ...
            if (in_array($resource, ['transactions', 'purchases', 'cashflow', 'users'])) {
                ...
                // RBAC: Cashiers cannot view full cashflow history (optional, but safer)
                // if ($resource === 'cashflow' && $currentUser['role'] === ROLE_CASHIER) {
                //    sendJson(['error' => 'Access denied'], 403);
                // }
            }
    ...
    case 'POST':
        ...
        try {
            $stmt = $pdo->prepare($sql);
            $stmt->execute($data);
            // If ID was not provided (auto-increment), get it. But here IDs are UUIDs usually provided or generated by client.
            // If client didn't provide ID, we might return the inserted data.
            sendJson($data, 201);
        } catch (Exception $e) {
    ...
    case 'PUT':
        ...
        // Hash password for users
        if ($resource === 'users' && isset($data['password'])) {
            // Only hash if it's not already hashed (simple check)
            // Or just assume client sends plain text.
            // Ideally client should only send password if it changed.
        ...
        try {
            ...
            // If not found, maybe try insert (upsert) if that's what the Node server did?
            // Node server did update, if not found returned 404.
            sendJson(['error' => $e->getMessage()], 500);
        }
        break;
